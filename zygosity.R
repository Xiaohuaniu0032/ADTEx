...
write.table(baf_file,file=paste(outputLoc,"/extracted.baf",sep=""),quote=F,sep="\t",
              row.names=F)

run_hmm(baf=baf_file,cnv_res=paste(outputLoc,"/cnv.result",sep=""),dest=paste(outputLoc,"/zygosity/",sep=""),chrom=chrom)
