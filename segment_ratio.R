...
write.table(result,file=paste(outputLoc,"/ratio.data",sep=""),quote=F,sep="\t",
            row.names=F)
cat(chrom,sep=",",file=paste(outputLoc,"/chrom",sep=""))
...
if(bafIn & !ploidyIn){
  ...
  write.table(segment.data$output[,2:6],file=paste(outputLoc,"/temp/snp_segments",sep=""),
